...
class InMemoryTreeNode<T> implements Serializable {
    ...
    public InMemoryTreeNode(final T id, final T parent, final int level, final boolean visible, final String description) {
        super();
        this.id = id;
        this.parent = parent;
        this.level = level;
    ...
    public synchronized InMemoryTreeNode<T> add(final int index, final T child,
        ...
        final InMemoryTreeNode<T> newNode = new InMemoryTreeNode<T>(child,
                getId(), getLevel() + 1, getId() == null ? true : visible, description);
