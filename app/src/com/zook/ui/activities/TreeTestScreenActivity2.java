...
import com.zook.tree.extended.InMemoryTreeStateManager;
import com.zook.tree.extended.TreeBuilder;
import com.zook.tree.extended.TreeStateManager;
import com.zook.tree.extended.TreeViewList;
...
import com.zook.ui.view.SyncTreeStandardAdapter2;
	
public class TreeTestScreenActivity2 extends Activity implements ITreeViewActivity {

	private final Set<Long> selected = new HashSet<Long>();

	private static final String TAG = TreeTestScreenActivity2.class.getSimpleName();
	...
	private TreeStateManager<Long, HashMap> manager = null;
	private SyncTreeStandardAdapter2 simpleAdapter;
	@SuppressWarnings("unused")
	...
	public void onCreate(final Bundle instanceState) {
		...
		manager = new InMemoryTreeStateManager<Long, HashMap>();
		final TreeBuilder<Long, HashMap> treeBuilder = new TreeBuilder<Long, HashMap>(manager);
		...
		setContentView(R.layout.sync_tree2);
		treeView = (TreeViewList) findViewById(R.id.syncTreeView2);

		simpleAdapter = new SyncTreeStandardAdapter2(this, selected, manager, LEVEL_NUMBER);
	 ...
	 */
	protected void populateNodeDescriptionsFromRemoteShare(TreeBuilder<Long, HashMap> treeBuilder) {
		...
		try {
			...
			for(int i=0; i<directoryContents.size(); i++){
				...
				map.put("description", directoryContents.get(i));
				treeBuilder.addRelation( null, (long)id, map);
	 ...
	 */
	public long addNodesToTree(TreeBuilder<Long, HashMap> treeBuilder, long id, String dir)
	{
		...
		try
		{
			...
			if (!cifsInteraction.isLeaf(dir))
			{
				...
				for(int i=0; i<directoryContents.size(); i++){
					...
					treeBuilder.addRelation( parentId, (long)childId, map);
