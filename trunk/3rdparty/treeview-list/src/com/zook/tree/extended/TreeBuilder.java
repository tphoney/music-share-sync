 ...
 * @param <T> 
 */
public class TreeBuilder<T,S> {
    private static final String TAG = TreeBuilder.class.getSimpleName();

    private final TreeStateManager<T,S> manager;

    private T lastAddedId = null;
    private int lastLevel = -1;

    public TreeBuilder(final TreeStateManager<T,S> manager) {
     ...
     *            child id
     */
    public synchronized void addRelation(final T parent, final T child, S params) {
        Log.d(TAG, "Adding relation parent:" + parent +  " -> child: " + child );
        manager.addAfterChild(parent, child, null, params);
        lastAddedId = child;
     ...
     *            its folder name
     */
    public synchronized void sequentiallyAddNextNode(final T id, final int level, S params) {
        Log.d(TAG, "Adding sequentiall node " + id + " at level " + level);
        if (lastAddedId == null) {
            addNodeToParentOneLevelDown(null, id, level, params);
        } else {
            if (level <= lastLevel) {
                final T parent = findParentAtLevel(lastAddedId, level - 1);
                addNodeToParentOneLevelDown(parent, id, level, params);
            } else {
                addNodeToParentOneLevelDown(lastAddedId, id, level, params);
            }
        }
    }
    ...
    private void addNodeToParentOneLevelDown(final T parent, final T id,
        ...
        manager.addAfterChild(parent, id, null, params);
